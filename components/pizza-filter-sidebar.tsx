...
export function PizzaFilterSidebar({ isOpen, onClose, onFilterChange }: PizzaFilterSidebarProps) {
  const [priceRange, setPriceRange] = useState([0, 500])
  const [selectedCategories, setSelectedCategories] = useState<string[]>([])
  const [selectedSizes, setSelectedSizes] = useState<string[]>([])
  const [selectedCrusts, setSelectedCrusts] = useState<string[]>([])

  const categories = ["Margarita", "Pepperoni", "Karışık", "Vejeteryan", "Sucuklu", "Ton Balıklı", "Mantarlı", "Acılı"]
                  ...
                  {crusts.map((crust) => (
                    <div key={crust} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => handleCrustChange(crust, checked as boolean)}
                      />
